  ...
  class TaxIdentifierRequest
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AvalaraSdk::EInvoicing::V1::TaxIdentifierRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AvalaraSdk::EInvoicing::V1::TaxIdentifierRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @identifier_type.to_s.length > 100
        invalid_properties.push('invalid value for "identifier_type", the character length must be smaller than or equal to 100.')
      end
    ...
    def valid?
      ...
      return false if @identifier_type.to_s.length > 100
      return false if @identifier.nil?
      return false if @identifier.to_s.length > 100
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] identifier_type Value to be assigned
    def identifier_type=(identifier_type)
      if identifier_type.nil?
        fail ArgumentError, 'identifier_type cannot be nil'
      end

      if identifier_type.to_s.length > 100
        fail ArgumentError, 'invalid value for "identifier_type", the character length must be smaller than or equal to 100.'
      end
    ...
    def hash
      [country_code, identifier_type, identifier, extensions].hash
    end
