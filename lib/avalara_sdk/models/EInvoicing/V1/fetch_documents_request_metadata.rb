...
module AvalaraSdk::EInvoicing
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `AvalaraSdk::EInvoicing::V1::FetchDocumentsRequestMetadata` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `AvalaraSdk::EInvoicing::V1::FetchDocumentsRequestMetadata`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'data_format')
        ...
        self.data_format = nil
      end

      if attributes.key?(:'data_format_version')
        self.data_format_version = attributes[:'data_format_version']
      else
    ...
    def list_invalid_properties
      ...
      if @country_mandate.nil?
        invalid_properties.push('invalid value for "country_mandate", country_mandate cannot be nil.')
      end
